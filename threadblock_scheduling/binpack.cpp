...
int main()
{
	...
	cout << "a app의 SM 개수 : " << infovec1[a].sm << ", a app의 tb 개수 : " << infovec1[a].tb << endl;
	cout << "b app의 SM 개수 : " << infovec2[b].sm << ", b app의 tb 개수 : " << infovec2[b].tb << endl;

	cout << "각 SM의 남은 자원량" << endl;

	for (int i = 0; i < 30; i++) {
		cout << "sm " << i << "번 reg : " << sms[i].reg << " 남음, smem : " << sms[i].smem << "남음" << endl;
	...
	for (int i = 0; i < 30; i++) {
		...
		cout << "c app의 " << i << "번째 SM TB 개수 : " << tb<<endl;
	}
