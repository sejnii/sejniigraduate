...
void get_info(string app_name, vector<double>* app);
...
int main()
{
	...
	cout << "Identification of SM-TB numbers" << endl << endl;

	cout << "1번 응용의 이름을 입력하세요" << endl;
	cin >> app_name[0];
	get_info(app_name[0], app1);

	cout << "2번 응용의 이름을 입력하세요" << endl;
	cin >> app_name[1];
	get_info(app_name[1], app2);

	if (concurrent_apps >= 3) {
		cout << "3번 응용의 이름을 입력하세요" << endl;
		cin >> app_name[2];
		get_info(app_name[2], app3);
	}
	if (concurrent_apps == 4) {
		cout << "4번 응용의 이름을 입력하세요" << endl;
		cin >> app_name[3];
		get_info(app_name[3], app4);
	}
...
//	while (1) {
	...
	//	while (1) {
			double sm_rate=0.01;
		/*	cout << "SM convergence를 판단할 rate을 입력하세요 ex> 0.05 -> 5%를 의미함" << endl;
			cin >> sm_rate;
			if (sm_rate == -1)
				break;*/
